...
# ==========================================
st.set_page_config(page_title="Operations & Intelligence Dashboard", page_icon="✈️", layout="wide")
...
def create_pdf_report(df_res, flight_num, date_str, buffer_info="", intelligence_info="", current_season="", seasonal_log_df=None):
    ...
    # Section 1: Full Flight Rotation Detailed Log
    pdf.set_font("Arial", 'B', 11)
    pdf.cell(0, 10, "1. Full Flight Rotation Detailed Log", ln=True)
    pdf.set_font("Arial", 'B', 7)
    cols = ["Flight", "Origin", "Dest", "STD", "ATD", "STA", "ATA", "Dep Dly", "Arr Dly", "Block", "Turn"]
    cw = [25, 20, 20, 22, 22, 22, 22, 25, 25, 30, 30]
    ...
    if intelligence_info:
        pdf.ln(5); pdf.set_font("Arial", 'B', 11); pdf.cell(0, 10, "3. Structural Intelligence & ML Prediction", ln=True)
        pdf.set_font("Arial", '', 9); pdf.multi_cell(0, 8, intelligence_info)

    if seasonal_log_df is not None and not seasonal_log_df.empty:
        pdf.add_page(); pdf.set_font("Arial", 'B', 11); pdf.cell(0, 10, "4. Full Seasonal Rotation Log (Historical Data)", ln=True)
...
if uploaded_file:
    try:
        ...
        reg_cols = [c for c in df.columns if "Registration" in c]
        if reg_cols: df['Registration_Final'] = df[reg_cols].bfill(axis=1).iloc[:, 0]
        else: st.error("No registration column."); st.stop()
        ...
        if not df_clean.empty:
            ...
            if st.session_state.get('run_main') and fa_api_key:
                flights_raw = fetch_unified_data(fa_api_key, registration, search_date, search_date)
                if flights_raw:
                    flights = [f for f in flights_raw if f and f.get('scheduled_out') is not None]
                    rows, prev_sta, prev_ata, prev_arr_delay = [], None, None, 0
                    for f in sorted(flights, key=lambda x: x.get('scheduled_out')):
                    ...
                    c_t2.metric("Aircraft", registration); c_t3.metric("Season", current_season_code); c_t4.metric("Date", search_date.strftime("%d-%m-%Y"))

                    st.subheader("📋 Flight Rotation Detailed Log")
                    with st.expander("ⓘ Info - Column definitions & Calculations"):
                        st.write("""
                        This table provides a high-resolution log of the aircraft's daily rotation.
                        - **STD/STA:** Commercial schedule times.
                        - **ATD/ATA:** Real-world movement times.
                        - **Dep/Arr Delay:** Calculation: (Actual - Scheduled).
                        - **Block (S/A):** Scheduled vs. Actual flying duration.
                        - **Turn (S/A):** Scheduled Turnaround vs. Actual Ground Time between flights.
                        """)
                    ...
                    st.subheader("📉 Delay Propagation Variance")
                    with st.expander("ⓘ Info"): st.write("Visualizes how arrival delays impact following departures. Red = Departure Delay, Dashed = Inbound Arrival Delay.")
                    ...
                    st.subheader("🕒 Execution vs. Schedule Timeline Variance")
                    with st.expander("ⓘ Info"): st.write("Gantt view. Grey = Plan, Colored = Actual. AMS margin: 45m (Black/Red), Others: 15m (Red).")
                    fig_g = go.Figure()
                    for i, r in df_res.iterrows():
                        ...
                        dest, delay = str(r['Dest']).upper(), r['Arr_Delay_Num']
                        if dest in ["AMS", "EHAM"]: color = '#EF4444' if delay > 45 else ('#000000' if 10 <= delay <= 45 else '#22C55E')
                    ...
                    target_num = "".join(filter(str.isdigit, flight_display_name.split()[-1]))
                    match_indices = [i for i, f_id in enumerate(df_res['Flight']) if target_num in str(f_id)]
                    df_log_subset = df_res.loc[:match_indices[0]].copy() if match_indices else df_res.copy()

                    with c1:
                        st.subheader("⌛ Ground Stop Efficiency")
                        with st.expander("ⓘ Info"): st.write("Comparison of planned vs. actual ground time for every unique stop.")
                    ...
                    with c2:
                        st.subheader("🚀 Airborne Performance Buffer")
                        with st.expander("ⓘ Info"): st.write("Scheduled vs. Actual flight duration. Shorter green bars indicate airborne efficiency.")
                    ...
                    st.divider(); st.subheader(f"🛡️ Total Turnaround Buffer (until {flight_display_name} Departure)")
                    buffer_summary = ""
                    if match_indices:
                    ...
                    st.download_button("📥 Download Report as PDF", pdf_basic, f"{flight_display_name}_{search_date}.pdf", "application/pdf", use_container_width=True)

                    st.divider(); st.subheader(f"🎯 Structural Pattern Recognition ({current_season_code})")
                    if st.button(f"🚀 Analyze structural {current_season_code} patterns", use_container_width=True):
                        with st.spinner("Analyzing seasonal patterns..."):
                            ...
                            api_ident = df_res.loc[match_indices[0], 'Flight'] if match_indices else registration
                                ...
                                if current_dt.weekday() == search_date.weekday():
                                    ...
                                    if day_f:
                                        for f in day_f:
                                            ...
                                            if dest and dest.get('code_iata') in targets: valid_reg = f.get('registration'); break
                                    if valid_reg:
                                        rotation_raw = fetch_unified_data(fa_api_key, valid_reg, current_dt, current_dt)
                                        if rotation_raw:
                                            sample_count += 1
                                            day_T, day_F, d_rows, prev_s, prev_a = 0, 0, [], None, None
                                            for h in sorted([f for f in rotation_raw if f and f.get('scheduled_out')], key=lambda x: x.get('scheduled_out')):
                                                ...
                                                t_loss = (t_a - t_s) if prev_s and prev_a else 0
                                                f_loss = (int((h_ata-h_atd).total_seconds()/60) - int((h_sta-h_std).total_seconds()/60)) if h_sta and h_ata and h_atd else 0
                                                ...
                                                if h.get('destination', {}).get('code_iata') in targets:
                                                    if h_sta and h_ata: ml_X.append([day_T, day_F]); ml_y.append(int((h_ata - h_sta).total_seconds()/60))
                                                    break
                                                prev_s, prev_a = h_sta, h_ata
                                            if d_rows:
                                                df_h = pd.DataFrame(d_rows); history_rows.append({'Date': current_dt.strftime('%d-%b'), 'Reg': valid_reg, 'Max Turn Loss': f"{df_h.loc[df_h.T_Var.idxmax()]['Origin']} (+{df_h.T_Var.max()}m)", 'Max Flight Loss': f"{df_h.loc[df_h.F_Var.idxmax()]['Ident']} (+{df_h.F_Var.max()}m)"})
                        ...
                        if history_rows:
                            ...
                            with st.expander("ⓘ Info - Column definitions & Calculations"):
                                ...
                                - **Date:** Analyzed weekday.
                                - **Reg:** Specific aircraft used.
                                - **Max Turn Loss:** Station where (Actual - Planned) turnaround time was highest.
                                - **Max Flight Loss:** Leg where (Actual - Planned) block time was highest.
                                """)
                            ...
                            if len(ml_y) > 2:
                                ...
                                with col_l:
                                    ...
                                    st.markdown("#### 🏢 Recurring Turnaround Bottlenecks")
                                    ...
                                    st.markdown("#### ✈️ Flight Leg Efficiency Bottlenecks")
                                ...
                                with col_r:
                                    p_m = f"Prediction: +{max(0, s_pred):.1f}m. R2: {r2:.2f}. Ground Factor: {g_i:.2f}x, Air Factor: {f_i:.2f}x."
                                    intelligence_text += "\n" + p_m
                                    st.markdown(f'<div class="ml-card"><h3>🔮 {next_season_code} Robustness Prediction</h3><div style="font-size: 3em; font-weight: bold; color: {"#ef4444" if s_pred > 15 else "#10b981"};">+{max(0, s_pred):.1f}m</div><hr><p><b>Onderbouwing:</b> R²: {r2:.2f}, Grond Factor: {g_i:.2f}x, Lucht Factor: {f_i:.2f}x.</p></div>', unsafe_allow_html=True)
                            ...
                            st.download_button("📥 Download Detailed Seasonal Report", pdf_full, f"Detailed_{flight_display_name}.pdf", "application/pdf", use_container_width=True)
